<div class="container">
	<div class="row">
		...
		<div class="file">
			<h3>Modifier le nom et l'emplacement du fichier</h3>
			<?php echo $this->Form->create("Upload"); ?>
			<?php echo $this->Form->input('name', array('label' => 'Nom du fichier :', 'value' => $filename)); ?>
			<?php echo $this->Form->create("Upload", ['url' => array('action' => 'move', 'controller' => 'uploads')]); ?>
			<label>Dossier :</label>
			<select class="" name="folder">
				<option value=""><?php echo $username; ?></option>
				<?php for ($i=0; $i < count($select) ; $i++) {
					$id = $select[$i]['folders']['id'];
